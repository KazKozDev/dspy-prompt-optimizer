...
from ..config import ComplexityLevel, OptimizerConfig, OptimizerType, TaskAnalysis
...
class OptimizerSelector:
    ...
    def select(
        ...
        if optimizer_type == OptimizerType.DISTILLATION:
            teacher_model = optimizer_model or "openai/gpt-5"
    ...
    def _select_optimizer_type(
        self, task_analysis: TaskAnalysis, dataset_size: int, quality_profile: str
    ) -> OptimizerType:
    ...
    def _select_params(
        self, optimizer_type: OptimizerType, dataset_size: int, quality_profile: str
    ) -> dict:
        ...
        params = base_params.get(
            optimizer_type, base_params[OptimizerType.BOOTSTRAP_FEW_SHOT]
        )

        if quality_profile == "HIGH_QUALITY":
            params["max_bootstrapped_demos"] = min(
                params["max_bootstrapped_demos"] + 2, 8
            )
            ...
            params["max_bootstrapped_demos"] = max(
                params["max_bootstrapped_demos"] - 1, 1
            )
