...
class TaskAnalyzer:
    ...
    TASK_KEYWORDS = {
        TaskType.CLASSIFICATION: [
            "classify", "categorize", "label", "tag", "sentiment", "detect",
            "identify type", "determine category", "sort into"
        ],
        TaskType.EXTRACTION: [
            "extract", "parse", "find", "identify", "locate", "pull out",
            "get entities", "named entity", "ner", "information extraction"
        ],
        TaskType.SUMMARIZATION: [
            "summarize", "summarise", "condense", "brief", "tldr", "abstract",
            "key points", "main ideas", "synopsis"
        ],
        TaskType.QA: [
            "answer question", "qa", "q&a", "respond to", "answer based on"
        ],
        TaskType.RAG: [
            "rag", "retrieval", "search", "knowledge base", "document",
            "based on context", "from documents", "look up"
        ],
        TaskType.REASONING: [
            "reason", "think", "analyze", "solve", "figure out", "deduce",
            "logic", "math", "calculate", "compute", "step by step"
        ],
        TaskType.GENERATION: [
            "generate", "create", "write", "compose", "draft", "produce",
            "article", "essay", "story", "content"
        ],
        TaskType.ROUTING: [
            "route", "direct", "forward", "assign", "dispatch", "triage"
        ],
        TaskType.CODE: [
            "code", "program", "script", "function", "implement", "debug",
            "fix bug", "refactor", "python", "javascript"
        ],
    }
    
    DOMAIN_KEYWORDS = {
        "legal": ["legal", "law", "contract", "court", "attorney", "litigation", "compliance"],
        "finance": ["finance", "financial", "bank", "investment", "stock", "trading", "accounting"],
        "medical": ["medical", "health", "patient", "diagnosis", "clinical", "doctor", "symptom"],
        "support": ["support", "customer", "ticket", "help desk", "service", "complaint"],
        "engineering": ["code", "software", "engineering", "developer", "technical", "api", "system"],
        "education": ["education", "learning", "student", "course", "teach", "academic"],
    ...
    COMPLEXITY_INDICATORS = {
        "high": [
            "complex", "multi-step", "multiple", "comprehensive", "detailed",
            "thorough", "in-depth", "advanced", "sophisticated"
        ],
        "low": [
            "simple", "basic", "quick", "straightforward", "easy", "single"
        ],
    }
    
    TOOL_INDICATORS = {
        "calculator": ["calculate", "compute", "math", "arithmetic", "formula", "equation"],
        "web_search": ["search", "look up", "find online", "current", "recent", "latest", "news"],
        "python_repl": ["code", "execute", "run", "script", "data processing"],
        "wikipedia": ["wikipedia", "encyclopedia", "factual", "historical", "biography"],
    }
    
    def analyze(
        self,
        task_description: str,
        dataset_sample: Optional[List[Dict]] = None
    ) -> TaskAnalysis:
        """
        ...
        suggested_template = self._suggest_template(task_type, needs_retrieval, needs_tools, needs_multi_stage)
    ...
    def _infer_fields(
        self,
        task_type: TaskType,
        dataset_sample: Optional[List[Dict]]
    ) -> tuple:
        """Infer input and output fields."""
        if dataset_sample and len(dataset_sample) > 0:
            sample = dataset_sample[0]
            input_fields = [k for k in sample.keys() if k not in ["output", "label", "answer", "result"]]
            output_fields = [k for k in sample.keys() if k in ["output", "label", "answer", "result"]]
    ...
    def _needs_retrieval(self, desc_lower: str, task_type: TaskType) -> bool:
        ...
        retrieval_keywords = ["retriev", "search", "knowledge base", "document", "context", "rag"]
        return any(kw in desc_lower for kw in retrieval_keywords)
    
    def _needs_chain_of_thought(self, task_type: TaskType, complexity: ComplexityLevel) -> bool:
        """Determine if task needs chain of thought."""
        cot_tasks = {TaskType.REASONING, TaskType.QA, TaskType.CODE, TaskType.EXTRACTION}
    ...
    def _needs_multi_stage(
        ...
        multi_stage_keywords = [
            "then", "after that", "followed by", "step 1", "step 2",
            "first", "second", "finally", "multi-step", "pipeline"
        ]
