...
class MetaAgent:
    ...
    def configure(
        ...
        if mode == "auto":
            config = self._auto_configure(
                config, task_analysis, len(dataset), quality_profile
            )
        else:
    ...
    def _auto_configure(
        ...
        metric_config = self.metric_selector.select(
            task_analysis, quality_profile, judge_model=self.optimizer_model
        )
        ...
        config.agent_reasoning.append(
            f"Optimizer: {optimizer_config.optimizer_type.value}"
        )
    ...
    def _manual_configure(
        self, config: AgentConfig, overrides: dict[str, Any]
    ) -> AgentConfig:
        """Apply manual configuration overrides."""
        if "pipeline_type" in overrides and overrides["pipeline_type"]:
            config.pipeline_config.pipeline_type = PipelineType(
                overrides["pipeline_type"]
            )
            config.agent_reasoning.append(
                f"Pipeline override: {overrides['pipeline_type']}"
            )
        ...
        if "optimizer_type" in overrides and overrides["optimizer_type"]:
            config.optimizer_config.optimizer_type = OptimizerType(
                overrides["optimizer_type"]
            )

        if (
            "max_bootstrapped_demos" in overrides
            and overrides["max_bootstrapped_demos"]
        ):
            config.optimizer_config.max_bootstrapped_demos = overrides[
        ...
        if overrides.get("enable_rag") or overrides.get("pipeline_type") == "rag":
            ...
            config.pipeline_config.retriever_type = overrides.get(
                "retriever_type", "faiss"
            )
            config.pipeline_config.retriever_k = overrides.get("retriever_k", 5)
            config.agent_reasoning.append(
                f"RAG enabled: {config.pipeline_config.retriever_type}, k={config.pipeline_config.retriever_k}"
            )

        # Distillation configuration
        if overrides.get("enable_distillation"):
            config.enable_distillation = True
            config.teacher_model = overrides.get("teacher_model", "openai/gpt-5")
            config.distillation_samples = overrides.get("distillation_samples", 100)
            config.agent_reasoning.append(
                f"Distillation enabled: teacher={config.teacher_model}, samples={config.distillation_samples}"
            )
    ...
    def validate_config(self, config: AgentConfig) -> list[str]:
        ...
        if config.metric_config.metric_type == MetricType.LLM_JUDGE:
            if not config.metric_config.llm_judge_model:
                warnings.append(
                    "LLM Judge metric selected but no judge model specified"
                )
...
def create_meta_agent(
    optimizer_model: str = "openai/gpt-5-mini", artifacts_dir: str = "data/artifacts"
) -> MetaAgent:
    ...
    """
    return MetaAgent(optimizer_model=optimizer_model, artifacts_dir=artifacts_dir)
