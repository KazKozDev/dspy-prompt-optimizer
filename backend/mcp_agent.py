...
def _get_chat_model(provider: str, model_name: str, temperature: float = 0.1) -> BaseChatModel:
...
class DSPyMCPAgent:
    ...
    ANALYSIS_PROMPT = """You are a DSPy optimization expert. Analyze this task and return JSON.
...
Return ONLY valid JSON."""
    ...
    def __init__(self, agent_model: str = "openai/gpt-4o-mini"):
        ...
        # Initialize LLM
        try:
            self.llm = _get_chat_model(provider, model)
    ...
    async def _analyze_task_llm(self, task: str) -> Dict:
        ...
        try:
            prompt = self.ANALYSIS_PROMPT.format(task=task)
    ...
    async def run_async(
        ...
        print(f"[MCP Agent] Starting optimization workflow")
        ...
        try:
            ...
            print(f"[MCP Agent] Selected metric: {analysis.get('metric_type', 'exact_match')}")
            ...
            sig_result = await self.mcp.call_tool("create_signature", {
            ...
            mod_result = await self.mcp.call_tool("create_module", {
                "module_type": module_type,
                "use_cot": analysis["needs_cot"]
            })
            
            step.status = "success" if "error" not in mod_result else "error"
            step.observation = f"Created {mod_result.get('module_type', 'unknown')} module"
            yield {"type": "step", "data": step.__dict__}
            ...
            lm_result = await self.mcp.call_tool("configure_lm", {
            ...
            data_result = await self.mcp.call_tool("prepare_dataset", {
                "dataset": dataset,
                "train_ratio": 0.7,
                "dev_ratio": 0.2
            })
            ...
            opt_result = await self.mcp.call_tool("select_optimizer", {
            ...
            print(f"[MCP Agent] Optimizer params: max_bootstrapped={max_bootstrapped}, max_labeled={max_labeled}, num_candidates={num_candidates}")
            ...
            yield {"type": "step", "data": step.__dict__}
            compile_result = await self.mcp.call_tool("run_optimization", {
                "metric_type": metric_type
            })
            ...
            eval_result = await self.mcp.call_tool("evaluate_program", {
            ...
            export_result = await self.mcp.call_tool("export_program", {
            ...
            step.observation = f"Exported to {export_result.get('artifact_id', 'unknown')}"
            yield {"type": "step", "data": step.__dict__}
            
            # ========== Final Result ==========
            final_accuracy = eval_result.get("test_accuracy", compile_result.get("dev_accuracy", 0))
            ...
            print(f"[MCP Agent] Optimization complete! Test accuracy: {final_accuracy:.1%}")
