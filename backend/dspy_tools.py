...
class DSPyToolsSession:
    ...
    def reset(self):
        ...
        self.task_analysis: dict | None = None
        self.signature_code: str | None = None
        self.signature_class: type | None = None
...
async def create_signature(
    input_fields: list[str], output_fields: list[str], task_description: str = ""
) -> dict:
...
async def prepare_dataset(
    dataset: list[dict], train_ratio: float = 0.7, dev_ratio: float = 0.2
) -> dict:
    ...
    print(
        f"[DSPy Tools] Detected dataset fields: {dataset_input_field} -> {dataset_output_field}"
    )
    ...
    if len(valid_dataset) < 3:
        return {
            "error": f"Not enough valid examples. Need at least 3, got {len(valid_dataset)}"
        }
    ...
    if _session.signature_class:
        ...
        if not sig_output_fields:
            sig_output_fields = (
                [list(sig_fields.keys())[-1]] if len(sig_fields) > 1 else ["output"]
            )
    else:
    ...
    print(
        f"[DSPy Tools] Mapping: dataset[{dataset_input_field}] -> signature[{sig_input_fields[0]}], dataset[{dataset_output_field}] -> signature[{sig_output_fields[0]}]"
    )
...
async def run_optimization(metric_type: str = "exact_match") -> dict:
    ...
    if _session.signature_class:
        ...
        output_field = (
            output_fields[0] if output_fields else list(sig_fields.keys())[-1]
        )
    else:
        output_field = "output"
    ...
    try:
        ...
        compiled = optimizer.compile(
            _session.module_instance, trainset=_session.train_set
        )
...
async def evaluate_program(num_samples: int = 50) -> dict:
    ...
    for i, ex in enumerate(_session.test_set[:total]):
        try:
            ...
            ex_output_field = (
                [k for k in ex.keys() if k not in ex.inputs().keys()][0]
                if ex.keys()
                else output_field
            )
            ...
            print(
                f"[Evaluate] #{i}: input='{input_val[:30]}...' expected='{expected}' predicted='{predicted}'"
            )
...
async def export_program(artifact_name: str = "") -> dict:
    ...
    artifact_id = (
        artifact_name
        or f"dspy_{_session.session_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    target_lm = _session.target_lm or f"ollama/{settings.model_defaults.ollama_chat}"
    if "/" in target_lm:
    ...
    if provider == "ollama":
        lm_config_code = (
            ...
            "if not base_url:\n"
            '    cfg = yaml.safe_load((Path(__file__).resolve().parents[3] / "config" / "model_config.yaml").read_text(encoding="utf-8"))\n'
            '    base_url = cfg.get("endpoints", {}).get("ollama_base_url")\n'
            f'lm = dspy.LM("ollama_chat/{model}", api_base=base_url)'
        )
    elif provider == "anthropic":
        lm_config_code = (
            f'lm = dspy.LM("anthropic/{model}", api_key=os.getenv("ANTHROPIC_API_KEY"))'
        )
    elif provider == "google" or provider == "gemini":
        lm_config_code = (
            f'lm = dspy.LM("google/{model}", api_key=os.getenv("GOOGLE_API_KEY"))'
        )
    else:
        lm_config_code = (
            f'lm = dspy.LM("openai/{model}", api_key=os.getenv("OPENAI_API_KEY"))'
        )
...
'''
    ...
    artifact_dir = (
        Path(__file__).resolve().parents[1] / "data" / "artifacts" / artifact_id
    )
