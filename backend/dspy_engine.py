...
4. Run inference:
   result = program({', '.join([f'{r}="..."' for r in task_analysis["input_roles"]])})"""
    ...
    def _analyze_task(self, task_description: str) -> Dict[str, Any]:
        ...
        # Infer task type
        task_type = "reasoning"
        if "classif" in desc_lower or "categoriz" in desc_lower or "label" in desc_lower:
        ...
        # Infer complexity
        complexity = "medium"
        if len(task_description) > 300 or "complex" in desc_lower or "multi" in desc_lower:
            complexity = "high"
        elif len(task_description) < 100:
            complexity = "low"
        
        # Determine if CoT is needed
        needs_cot = task_type in ["reasoning", "extraction", "RAG"] or complexity == "high"
...
'''
    ...
    async def _run_compilation(
        ...
        try:
            ...
            # Parse provider from model string (e.g., "openai/gpt-4o" or "ollama/llama3.2:3b")
            if "/" in target_lm:
            ...
            elif optimizer_type == "BootstrapFewShotWithRandomSearch" and BootstrapFewShotWithRandomSearch:
