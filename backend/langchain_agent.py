...
def _get_chat_model(provider: str, model_name: str, api_key: Optional[str] = None, temperature: float = 0.2) -> BaseChatModel:
...
class RegisterTargetLMInput(BaseModel):
    target_lm_name: str = Field(description="Name of target LLM (e.g., openai/gpt-4o, ollama/llama3.1:8b)")
...
class DSPyLangChainAgent:
    ...
    def __init__(
        ...
        if "/" in agent_model:
            self.provider, self.model_name = agent_model.split("/", 1)
        else:
        ...
        # Initialize LLM
        self.llm = _get_chat_model(self.provider, self.model_name, temperature=temperature)
        ...
        def run_compilation(program_id: str, optimizer_type: str) -> str:
            ...
            try:
                ...
                if provider == "ollama":
                    ...
                    lm = dspy_mod.LM(f"ollama_chat/{model_name}", api_base=ollama_base_url)
                elif provider == "anthropic":
                ...
                for role in output_roles:
                    sig_fields[role] = dspy_mod.OutputField(desc=f"{role} from the model")
                ...
                n = len(full_dataset)
                train_size = self.state.data_splits.get("train", int(n * 0.7)) if self.state.data_splits else int(n * 0.7)
                dev_size = self.state.data_splits.get("dev", int(n * 0.2)) if self.state.data_splits else int(n * 0.2)
                ...
                for item in train_data:
                    ...
                    trainset.append(dspy_mod.Example(**example_dict).with_inputs(*input_roles))
                ...
                if optimizer_type == "BootstrapFewShot" and BootstrapFewShot:
                    ...
                    optimizer = BootstrapFewShotWithRandomSearch(
                        ...
                        max_labeled_demos=optimizer_params.get("max_labeled_demos", 16),
                        max_rounds=optimizer_params.get("max_rounds", 1),
                        num_candidate_programs=optimizer_params.get("num_candidate_programs", 12),
                    )
                ...
                for item in (dev_data or train_data[:10]):
                    ...
                    if len(output_roles) == 1:
                        example_dict[output_roles[0]] = item.get("output", "")
                    eval_examples.append(dspy_mod.Example(**example_dict).with_inputs(*input_roles))
...
'''
    ...
    def run(
        ...
        if self.use_langgraph and self.agent:
            return self._run_with_langgraph(business_task, target_lm, dataset, quality_profile)
        else:
...
Return the final artifact ID when complete."""
    ...
    def _run_manual(
        ...
        try:
            ...
            tools_by_name["analyze_business_goal"].invoke({"task_description": business_task})
            ...
            tools_by_name["assemble_program_pipeline"].invoke({
                ...
                "needs_chain_of_thought": analysis.get("needs_chain_of_thought", False),
            ...
            tools_by_name["run_compilation"].invoke({
                ...
                "optimizer_type": self.state.optimizer_type or "BootstrapFewShotWithRandomSearch"
            })
    ...
    async def run_async(
        ...
        try:
            # 1. Analyze business goal
            yield {"type": "step", "data": {"name": "Analyze Business Goal", "status": "running"}}
            tools_by_name["analyze_business_goal"].invoke({"task_description": business_task})
            yield {"type": "step", "data": self.state.steps[-1]}
            
            # 2. Register target LM
            yield {"type": "step", "data": {"name": "Register Target LM", "status": "running"}}
            tools_by_name["register_target_lm"].invoke({"target_lm_name": target_lm})
            yield {"type": "step", "data": self.state.steps[-1]}
            
            # 3. Define signature
            analysis = self.state.task_analysis or {}
            yield {"type": "step", "data": {"name": "Define Signature", "status": "running"}}
            tools_by_name["define_contract_signature"].invoke({
                "input_roles": analysis.get("input_roles", ["input"]),
                "output_roles": analysis.get("output_roles", ["output"]),
                "task_type": analysis.get("task_type", "classification"),
            ...
            # 4. Assemble pipeline
            yield {"type": "step", "data": {"name": "Assemble Pipeline", "status": "running"}}
            tools_by_name["assemble_program_pipeline"].invoke({
                "task_type": analysis.get("task_type", "classification"),
                "needs_retrieval": analysis.get("needs_retrieval", False),
                "needs_chain_of_thought": analysis.get("needs_chain_of_thought", False),
            ...
            # 5. Prepare data splits
            yield {"type": "step", "data": {"name": "Prepare Data", "status": "running"}}
            tools_by_name["prepare_eval_splits"].invoke({
                "train_ratio": 0.7,
                "dev_ratio": 0.2
            })
            ...
            # 6. Select optimizer
            yield {"type": "step", "data": {"name": "Select Optimizer", "status": "running"}}
            tools_by_name["select_compiler_strategy"].invoke({
                "task_type": analysis.get("task_type", "classification"),
                "complexity_level": analysis.get("complexity_level", "medium"),
                "data_size": len(dataset),
            ...
            # 7. Run compilation
            yield {"type": "step", "data": {"name": "Run Compilation", "status": "running"}}
            tools_by_name["run_compilation"].invoke({
                "program_id": self.state.program_id or "prog_default",
                "optimizer_type": self.state.optimizer_type or "BootstrapFewShotWithRandomSearch"
            })
            ...
            # 8. Log artifacts
            yield {"type": "step", "data": {"name": "Log Artifacts", "status": "running"}}
            tools_by_name["log_artifacts"].invoke({})
            yield {"type": "step", "data": self.state.steps[-1]}
