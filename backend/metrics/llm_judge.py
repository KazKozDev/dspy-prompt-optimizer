...
class LLMJudgeMetric(BaseMetric):
    ...
    JUDGE_PROMPTS = {
        "correctness": """You are an expert judge evaluating the correctness of an AI response.
...
- 0.0: Completely wrong""",
        ...
        "faithfulness": """You are an expert judge evaluating the faithfulness of an AI response to source context.
...
- 0.0: Completely unfaithful/hallucinated""",
        ...
        "coherence": """You are an expert judge evaluating the coherence and quality of an AI response.
...
- 0.0: Very poor, incoherent""",
        ...
        "custom": """You are an expert judge evaluating an AI response.
...
{{"score": <0.0-1.0>, "reasoning": "<brief explanation>"}}"""
    ...
    def __init__(
        ...
        Args:
            ...
            judge_model: Model to use for judging (e.g., "openai/gpt-4o-mini")
    ...
    def _get_llm(self):
        ...
        if self._llm is None:
            ...
            if provider == "openai":
                ...
                self._llm = dspy.LM(f"openai/{model_name}", api_key=api_key, temperature=self.temperature)
            elif provider == "anthropic":
                api_key = os.getenv("ANTHROPIC_API_KEY")
                self._llm = dspy.LM(f"anthropic/{model_name}", api_key=api_key, temperature=self.temperature)
            elif provider == "ollama":
                base_url = os.getenv("OLLAMA_BASE_URL", "http://localhost:11434")
                self._llm = dspy.LM(f"ollama_chat/{model_name}", api_base=base_url, temperature=self.temperature)
            else:
    ...
    def _build_prompt(
        ...
        template = self.JUDGE_PROMPTS.get(self.judge_type, self.JUDGE_PROMPTS["correctness"])
    ...
    def _parse_response(self, response: str) -> Dict[str, Any]:
        ...
        return {"score": 0.5, "reasoning": f"Could not parse response: {response[:200]}"}
    ...
    def __call__(
        ...
        task = self.get_field_value(example, "text") or self.get_field_value(example, "input")
...
class CorrectnessJudge(LLMJudgeMetric):
    ...
    def __init__(self, judge_model: str = "openai/gpt-4o-mini", output_field: str = "result"):
        super().__init__(
