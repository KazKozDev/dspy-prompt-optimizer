...
def _get_chat_model(provider: str, model_name: str, temperature: float = 0.2) -> BaseChatModel:
...
class DSPyMCPTools:
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Dict:
        ...
        try:
            if name == "analyze_task":
                ...
                return await self._evaluate_program(
                    arguments.get("num_samples", 50)
                )
            elif name == "export_program":
                return await self._export_program(
    ...
    async def _create_signature(self, input_fields: List[str], output_fields: List[str], task_description: str = "") -> Dict:
...
'''
        ...
        return {"status": "success", "signature_code": code, "input_fields": input_fields, "output_fields": output_fields}
    ...
    async def _create_module(self, module_type: str, use_cot: bool = False) -> Dict:
        ...
        if module_type == "ChainOfThought" or use_cot:
            self.session["module_instance"] = dspy.ChainOfThought(self.session["signature_class"])
            actual_type = "ChainOfThought"
        elif module_type == "ReAct":
            self.session["module_instance"] = dspy.ReAct(self.session["signature_class"])
            actual_type = "ReAct"
        else:
            self.session["module_instance"] = dspy.Predict(self.session["signature_class"])
            actual_type = "Predict"
    ...
    async def _prepare_dataset(self, dataset: List[Dict], train_ratio: float = 0.7, dev_ratio: float = 0.2) -> Dict:
        ...
        if self.session["task_analysis"]:
            input_fields = self.session["task_analysis"].get("recommended_input_fields", ["input"])
            output_fields = self.session["task_analysis"].get("recommended_output_fields", ["output"])
        else:
            input_fields = ["input"]
            output_fields = ["output"]
    ...
    async def _select_optimizer(self, optimizer_type: str, max_bootstrapped_demos: int = 4, 
                                 max_labeled_demos: int = 4, num_candidate_programs: int = 10) -> Dict:
        """Select optimizer."""
        ...
        return {"status": "success", "optimizer_type": optimizer_type, "config": self.session["optimizer_config"]}
    ...
    async def _run_optimization(self, metric_type: str = "exact_match") -> Dict:
        ...
        if self.session["task_analysis"]:
            output_field = self.session["task_analysis"].get("recommended_output_fields", ["output"])[0]
        else:
            output_field = "output"
        ...
        try:
            ...
            compiled = optimizer.compile(
                self.session["module_instance"],
                trainset=self.session["train_set"]
            )
    ...
    async def _evaluate_program(self, num_samples: int = 50) -> Dict:
        ...
        for ex in self.session["test_set"][:total]:
            try:
                input_field = list(ex.inputs().keys())[0]
                pred = self.session["compiled_program"](**{input_field: getattr(ex, input_field)})
        ...
        return {"status": "success", "test_accuracy": accuracy, "correct": correct, "total": total}
    ...
    async def _export_program(self, artifact_name: str = "") -> Dict:
        ...
        import uuid
        artifact_id = artifact_name or f"dspy_{uuid.uuid4().hex[:8]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
'''
    ...
    def _get_session_status(self) -> Dict:
        ...
        return {
            ...
            "module_type": self.session["module_type"] if self.session["module_instance"] else None,
...
class MCPLangChainAgent:
    ...
    ANALYSIS_PROMPT = """Analyze this business task for DSPy optimization.
...
Return ONLY valid JSON, no markdown or explanation."""
    ...
    def __init__(self, agent_model: str = "openai/gpt-5-mini"):
        ...
        self.llm = _get_chat_model(provider, model, temperature=0.1)
    ...
    async def run_async(
        ...
        messages = [
            ...
            HumanMessage(content=f"""Configure and optimize DSPy for this task:
...
Start by analyzing the task, then proceed through the optimization workflow.""")
        ...
        while iteration < max_iterations:
            ...
            try:
                ...
                step.status = "success"
                step.observation = response_text[:200] + "..." if len(response_text) > 200 else response_text
                yield {"type": "step", "data": step.__dict__}
                ...
                if tool_call:
                    ...
                    messages.append(HumanMessage(content=f"Tool result: {json.dumps(result)}"))
                    ...
                    if tool_name == "export_program" and "error" not in result:
                        ...
                        final_result = {
                            ...
                            "eval_results": {
                                ...
                                "metric_value": self.tools.session["eval_results"].get("test_accuracy", 
                                               self.tools.session["eval_results"].get("dev_accuracy", 0)),
                                "real_dspy": True
                            },
                    ...
                    if "complete" in response_text.lower() or "finished" in response_text.lower():
                    ...
                    messages.append(HumanMessage(content="Please continue with the next step. Call a tool using JSON format: {\"tool\": \"tool_name\", \"arguments\": {...}}"))
        ...
        yield {"type": "error", "error": "Max iterations reached without completing optimization"}
