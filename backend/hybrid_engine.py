...
from agent import MetaAgent, AgentConfig, PipelineType, MetricType, OptimizerType
from agent.config import TaskAnalysis
from metrics import (
    BaseMetric, ExactMatchMetric, TokenF1Metric, 
    LLMJudgeMetric, SemanticSimilarityMetric
)
from pipelines import PipelineBuilder
from pipelines.templates import build_from_template
...
class HybridDSPyEngine:
    ...
    async def run_async(
        ...
        try:
            ...
            self._update_step(
                ...
                observation=f'{len(warnings)} warnings' if warnings else 'Configuration valid',
                duration_ms=50
            )
            yield {"type": "step", "step": asdict(step)}
            ...
            self._update_step(
                step,
                status="success" if compilation_result.get("status") == "success" else "error",
            ...
            if self.config.enable_distillation:
                ...
                self._update_step(
                    step,
                    status="success" if distillation_result.get("status") == "success" else "error",
            ...
            program_code = self._generate_program_code(compilation_result, distillation_result)
            ...
            result = {
                ...
                "task_analysis": self.config.task_analysis.__dict__ if self.config.task_analysis else {},
    ...
    def _create_metric(self) -> BaseMetric:
        ...
        output_field = self.config.task_analysis.output_fields[0] if self.config.task_analysis.output_fields else "result"
        ...
        if metric_type == MetricType.EXACT_MATCH:
            ...
            return LLMJudgeMetric(
                ...
                judge_model=self.config.metric_config.llm_judge_model or "openai/gpt-4o-mini",
            ...
            return SemanticSimilarityMetric(
                output_field=output_field,
                model_name=self.config.metric_config.semantic_model or "all-MiniLM-L6-v2"
            )
    ...
    async def _run_compilation(
        ...
        try:
            dspy, BootstrapFewShot, BootstrapFewShotWithRandomSearch, MIPROv2, COPRO = _load_dspy()
            ...
            dev_data = dataset[train_size:train_size + dev_size] if dev_size > 0 else []
            ...
            elif optimizer_type == OptimizerType.BOOTSTRAP_RANDOM_SEARCH and BootstrapFewShotWithRandomSearch:
            ...
            if dev_data:
                for item in dev_data:
                    ...
                    if len(output_fields) >= 1:
                        example_dict[output_fields[0]] = item.get("output", "")
                    eval_examples.append(dspy.Example(**example_dict).with_inputs(*input_fields))
            else:
            ...
            for example in eval_examples[:eval_count]:
                try:
                    input_kwargs = {field: getattr(example, field) for field in input_fields}
    ...
    def _generate_program_code(self, compilation_result: Dict, distillation_result: Optional[Dict] = None) -> str:
        ...
        predictor_type = "ChainOfThought" if pipeline_type in [PipelineType.CHAIN_OF_THOUGHT, PipelineType.RAG] else "Predict"
...
'''
    ...
    async def _run_distillation(self, dataset: List[Dict], target_lm: str) -> Dict[str, Any]:
        """Run teacher-student distillation."""
        start_time = time.time()
        
        try:
            from distillation import TeacherStudentDistiller, DistillationConfig
            ...
            input_field = self.config.task_analysis.input_fields[0] if self.config.task_analysis.input_fields else "input"
            seed_inputs = [item.get("input", item.get(input_field, "")) for item in dataset[:10]]
            
            generated_samples = await distiller.generate_samples_async(
                seed_inputs=seed_inputs,
                task_description=self.config.business_task
            )
