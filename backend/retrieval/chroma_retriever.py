...
class ChromaRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        self.embedding_model = (
            embedding_model or get_settings().model_defaults.semantic_model
        )
    ...
    def index(self, documents: list[str], metadata: list[dict] | None = None) -> None:
        ...
        if metadata is None:
            ...
            metadata = [{k: str(v) for k, v in (m or {}).items()} for m in metadata]
    ...
    def update(
        ...
        collection.update(ids=ids, documents=documents, metadatas=metadata)
