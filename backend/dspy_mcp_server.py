        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "model_name": {
                        "type": "string",
                        "description": "Model name with provider prefix (e.g., 'ollama/gemma3:4b', 'openai/gpt-5')",
                    }
                },
                "required": ["model_name"],
            },
    ...
    try:
        if name == "analyze_task":
            ...
            result = await _create_module(
                arguments.get("module_type", "Predict"), arguments.get("use_cot", False)
            )
            ...
            result = await _evaluate_program(arguments.get("num_samples", 50))
        elif name == "export_program":
            result = await _export_program(arguments.get("artifact_name", ""))
...
async def _analyze_task(task_description: str) -> dict:
    ...
    if word_count > 50 or any(
        w in task_lower for w in ["complex", "multi-step", "reasoning"]
    ):
        complexity = "high"
    elif word_count > 20:
        complexity = "medium"
...
async def _prepare_dataset(
    ...
    if _session.task_analysis:
        ...
        output_fields = _session.task_analysis.get(
...
async def _run_optimization(metric_type: str = "exact_match") -> dict:
    ...
    # Define metric
    if _session.task_analysis:
        output_field = _session.task_analysis.get(
            "recommended_output_fields", ["output"]
        )[0]
    else:
        output_field = "output"
    ...
    try:
        if optimizer_type == "BootstrapFewShot":
            ...
            optimizer = tp.MIPROv2(
                metric=metric, num_candidates=config.get("num_candidate_programs", 10)
            )
        elif optimizer_type == "COPRO":
            optimizer = tp.COPRO(metric=metric)
        else:
